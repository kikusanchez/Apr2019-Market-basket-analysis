...
#searching pattern ("Monitors.") into the vector (master_products)
grep("Monitors.", master_products)


#replacing the names of the values in the vector that contains "Monitors." into the word "monitor"
master_products[grep("Monitors.",master_products)]<-"monitor"
master_products


#adding to transactions the products (replaced by his category name) into a new label called 'categories'
electronidex@itemInfo$categories <- master_products

str(electronidex)

# 
# monitors<-grepl("Monitors.", all_products)
# 
# if (grepl("Monitors.", products)){
#   print("OHH")
# }
# 

##important code (it works)
vector_labels<-itemLabels(electronidex)
vector_labels
grep("Laptop",vector_labels)
vector_labels[grep("Apple",vector_labels)]<-"Mac"
print(vector_labels)
electronidex@itemInfo$categories<-vector_labels
str(electronidex)
##
...
#Rules
rules<- apriori(electronidex, parameter = list(supp=0.005, conf=0.7, minlen=1))
inspect(rules)
...
TempDFB2B <-
  as.data.frame(b2b@itemInfo$labels )
...
TempDFB2B2 <-
  merge(TempDFB2B, categories, by.x="ProdName", by.y="ProdName")
...
Rules_b2b_categories<- apriori (B2B_categories, parameter = list(maxlen=3, supp = 0.003, conf = 0.6, minlen=1))


#looking for redundant rules
is.redundant(Rules_b2b_categories)

#show if there are redundant rules
which(is.redundant(Rules_b2b_categories), arr.ind = TRUE, useNames = TRUE)

#removing redundant rules
Rules_b2b_categories <- Rules_b2b_categories[!is.redundant(Rules_b2b_categories)]

#confirming there are'nt more redundant rules
is.redundant(Rules_b2b_categories)#rules.sorted<-sort(Rules_b2b_categories, by="lift")
#inspect(Rules_b2b_categories)

ruleExplorer(Rules_b2b_categories)



#RULES FILTERING BY BRANDS
#reading the csv cointaining all products with their category in the name
master_products2<-read.csv("../market_basket_analysis/info/Master_products_categories.csv",header = FALSE, sep = ",")
...
#Rules b2b by brands without agreggate function
Rules_b2b_brands_<- apriori (b2b, parameter = list(supp = 0.005, conf = 0.7, minlen=1))


ruleExplorer(Rules_b2b_brands_)
...
TempDFB2C <-
  as.data.frame(b2c@itemInfo$labels )
...
#Rules b2b
Rules_b2c<- apriori (B2C_categories, parameter = list(supp = 0.005, conf = 0.7, minlen=1))
Rules_b2c<- apriori (b2c, parameter = list(supp = 0.001, conf = 0.6, minlen=1))
#rules.sorted<-sort(Rules_b2c, by="lift")

inspect(B2C_categories)

ruleExplorer(B2C_categories)
ruleExplorer(Rules_b2c)
#looking for redundant rules
is.redundant(Rules_b2c)
...
#Rules b2b by categories
Rules_b2b_categories<- apriori (B2B_categories, parameter = list(supp = 0.01, conf = 0.7, minlen=1))

#rules.sorted<-sort(Rules_b2b_categories, by="lift")
inspect(Rules_b2b_categories)
